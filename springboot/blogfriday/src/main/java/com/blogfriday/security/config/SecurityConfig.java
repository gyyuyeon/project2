...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize-> authorize
				                   ...
				                   //.requestMatchers("/","/api/**","/api/product/**","/images/**","/user/signup","/user/login","/board/list/**","/swagger-ui/**","/v3/api-docs/**").permitAll() //로그인 없이접근 허용한다.
	             ...
	             if(accessToken != null) {
	                 ...
	                 try {
	                     Claims claims = Jwts.parser().verifyWith((SecretKey)key)
	                             ...
	                             .getPayload();
	                     String userIdEmail = (String) claims.get("userIdEmail");	                  
	                     tokenService.deleteTokens(userIdEmail);
	                 } catch (Exception e) {
